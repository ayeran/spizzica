$(function(){
	...
	function start(){
		...
		$container.find('.pd_photo').each(function(i){
			...
			$('<img />').load(function(){
				...
				var param={	'top': Math.floor(Math.random()*verticalMax)+'px',
	...
	function mouseDown($photo){
		...
		if(ie){
			var param={'width':'+=40px','height':'+=40px'};
		}else{
			var param={	'width':'+=40px',
						'height':'+=40px',
						'rotate':'0deg',
						'shadow':'5px 5px 15px #222'};
		};
		$photo.stop(true,true).animate(param,100).find('img').stop(true,true).animate({'width':'+=40px','height':'+=40px'},100)};
		$(document).bind('mouseup',
			function(e){
				if(mouseup){
					...
					var newTop=$photoT+r;var newLeft=$photoL+r;
					if(ie){
						var param={	'width':'-=40px',
									'height':'-=40px',
									'top':newTop+'px',
					...
					$photo.stop(true,true).animate(param,200).find('img').stop(true,true).animate({'width':'-=40px','height':'-=40px'},200)
				};
				e.preventDefault()
			}
		);
		$container.find('.delete').bind('click',
			function(){
				var $photo=$(this).parent();
				var $photoT=parseFloat($photo.css('top'),10);
				var $photoL=parseFloat($photo.css('left'),10);
				var $photoZIndex=$photo.css('z-index');
				var $trash=$('<div />',{'className':'pd_paperball',
					'style':'top:'+parseInt($photoT+photoH/2)+'px;left:'+parseInt($photoL+photoW/2)+'px;width:0px;height:0px;z-index:'+$photoZIndex}).appendTo($container);
				$trash.animate({'width':photoW+'px',
					'height':photoH+'px',
					'top':$photoT+'px',
					'left':$photoL+'px'},100,function(){
						var $this=$(this);
						setTimeout(function(){
							$this.remove()
						},800
				...
				$photo.animate({'width':'0px',
					'height':'0px',
					'top':$photoT+photoH/2+'px',
					'left':$photoL+photoW/2+'px'},200,function(){
						--photosSize;
						$(this).remove();}
				)
			}
		...
		function stack(){
			...
			$container.find('.pd_photo').each(function(i){
				var $photo=$(this);
				$photo.css('z-index',parseInt(findHighestZIndex())+1000+i).stop(true).animate({'top':parseInt((windowsH-100)/2-photoH/2)+'px','left':parseInt((windowsW-100)/2-photoW/2)+'px'},800,function(){
					...
					$photo.prepend($nav);
					$photo.draggable('destroy').find('.delete').hide().andSelf().find('.pd_hold').unbind('mousedown').bind('mousedown',function(){return false});
		...
		function enlarge($photo){
			...
			$photo.animate(param,500,
				function(){
					idxLarge=$(this).index();
					var $photo=$(this);
					$photo.unbind('mousemove').bind('mousemove',
					function(){
						$(this).find('.pd_next_photo').show()}).unbind('mouseleave').bind('mouseleave',
							function(){
								$(this).find('.pd_next_photo').hide();
							}
						)
				}
			).find('img').animate({'width':'+=200px','height':'+=200px'},500)
		}
		function disperse(){
			var windowsW=$(window).width();
			var windowsH=$(window).height();
			$container.find('.pd_photo').each(
				function(i){
					...
					if($photo.index()==idxLarge){
						if(ie){
							var param={'top':parseInt((windowsH-100)/2-photoH/2)+'px',
